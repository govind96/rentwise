...
type Portal = {
  ...
  submissions: Array<{ id: number; amount: number; paid_on: string; mode: string; reference: string | null; created_at: string }>;
};
...
export default function TenantPortal() {
  ...
  async function submitPayment(event: FormEvent<HTMLFormElement>) {
    event.preventDefault(); setMessage(''); const response = await authFetch('/api/tenant/payment', { method: 'POST', body: new FormData(event.currentTarget) });
    const data = await response.json().catch(() => ({})) as { error?: string };
    if (!response.ok) { setMessage(data.error || 'Payment submission failed.'); return; }
    event.currentTarget.reset(); setMessage('Payment submitted — your manager will confirm it and issue a receipt.'); void load();
  }
