...
const viewCopy: Record<View, { eyebrow: string; title: string; subtitle: string }> = {
  overview: { eyebrow: 'SATURDAY, 22 AUGUST', title: 'Villa 26, at a glance', subtitle: 'The numbers, people and tasks that need your attention today.' },
  ...
  return (
    <div className="shell">
      <aside className="side">
        ...
        <div className="side-bottom"><div className="owner-chip"><span>GK</span><div><strong>Govind Kumar</strong><small>Owner workspace</small></div><b>•••</b></div><p className="no-login">Private MVP · No login required</p></div>
      </aside>
...
function Overview({ tenants, orders, metrics, availableBeds, onView, onTenant, onPayment }: { tenants: Tenant[]; orders: WorkOrder[]; metrics: ReturnType<typeof summaryShape>; availableBeds: number; onView: (view: View) => void; onTenant: (id: number) => void; onPayment: (id: number) => void }) {
  ...
  return <div className="view-stack">
    ...
    <section className="overview-lower">
      ...
      <article className="surface activity"><div className="surface-head"><div><p className="overline">RECENT ACTIVITY</p><h2>Latest at Villa 26</h2></div></div><div className="timeline"><div><i className="pay">₹</i><p><strong>Payment recorded</strong><span>₹1,000 from Meera Kumari</span></p><time>9:08 AM</time></div><div><i className="move">↳</i><p><strong>New allotment</strong><span>Kabita moved into Room 23 · B</span></p><time>Yesterday</time></div><div><i className="fix">◇</i><p><strong>Repair assigned</strong><span>Room 21 fan · Ramesh Electric</span></p><time>Yesterday</time></div></div></article>
    </section>
  </div>;
}
...
function PropertyView({ tenants, onTenant, onAdd }: { tenants: Tenant[]; onTenant: (id: number) => void; onAdd: () => void }) {
  return <div className="view-stack"><section className="property-hero"><div><span className="property-badge">V26</span><div><p className="overline">ACTIVE PROPERTY</p><h2>Villa 26</h2><p>Sunshine Signature Layout, Bengaluru</p></div></div><div className="property-facts"><div><span>13</span><small>Rooms</small></div><div><span>28</span><small>Beds</small></div><div><span>{tenants.length}</span><small>Tenants</small></div><div><span>{Math.round(tenants.length / 28 * 100)}%</span><small>Occupied</small></div></div><button className="quiet-button">Edit property</button></section>
